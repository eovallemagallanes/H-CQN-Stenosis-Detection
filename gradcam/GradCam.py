    ...
    def __call__(self, x):
        ...
        for i in range(x.size(0)):
            ...
            for name, module in self.model.named_children():
                #print(name)
                if name in ['normalization', 'q_layers', 'q_classifier'] or isinstance(module, torch.nn.Linear):
                    #print(name, 'view 1')
                    feature = feature.view(feature.size(0), -1)
                #elif name in ['q_layers']:
                #    print(name, 'view 2')
                #    feature = feature.view(256, 2)

                #print(module, feature.size())
                feature = module(feature)
                #print(feature.size())

                #print(name, feature.size())
            ...
            classes = softmax(feature)
            #print('classes: ', classes)
            #print('after sigmoid')
            #print('stenosis_prob: ', classes[0][1])
            stenosis_prob = classes[0][1]
            ...
            feature_map = np.float32(cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_TURBO))
